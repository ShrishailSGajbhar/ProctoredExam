...
def register():
    ...
    if request.method == 'POST' and form.validate():
        ...
        cur.execute('INSERT INTO users(username,name,email, password,confirmed,type) values(%s,%s,%s,%s,1,%s)',
...
def create_test():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        try:
            ...
            cur.execute(
                'INSERT INTO teachers (username, test_id, start, end, duration, show_ans, password) values(%s,%s,%s,'
                '%s,%s,%s,%s)',
...
def marks_calc(username, testid):
    if username == session['username']:
        ...
        if results['neg_mark'] == 1:
            ...
            results = cur.execute(
                "select sum(marks) as totalmks from students s,questions q where s.username=%s and s.test_id=%s and s.qid=q.qid and s.test_id=q.test_id and s.ans=q.ans",
                (username, testid))
            results = cur.fetchone()
            if str(results['totalmks']) == 'None':
                results['totalmks'] = 0
